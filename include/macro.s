...
// ============================================================================
// OS-GRADE SAFE STRUCTS (Rust-like Compile-Time Memory Enforcements)
// ============================================================================

/**
 * [safe_struc] / [end_safe_struc]
 * Purpose: Define a strongly-typed memory structure with strict bounds.
 * Usage:
 *   safe_struc PageTableEntry
 *      u8  Present,      1
 *      u64 PhysicalAddr, 1
 *   end_safe_struc
 */
%macro safe_struc 1
    %push   safe_struc
    %define %$strucname %1
    %assign %$strucoffset 0
%endmacro

%macro end_safe_struc 0
    %ifctx safe_struc
        %assign %$strucsize %$strucoffset
        // Export the total size of the struct as StructName_size
        %xdefine %$strucname %+ _size %$strucsize
        %pop    safe_struc
    %else
        %error "end_safe_struc without safe_struc"
    %endif
%endmacro
...
%macro u8 2
    %xdefine %$strucname %+ . %+ %1 %$strucoffset
    %xdefine %$strucname %+ _ %+ %1 %+ _bytes 1
    %assign %$strucoffset %$strucoffset + (1 * %2)
%endmacro

%macro u16 2
    %xdefine %$strucname %+ . %+ %1 %$strucoffset
    %xdefine %$strucname %+ _ %+ %1 %+ _bytes 2
    %assign %$strucoffset %$strucoffset + (2 * %2)
%endmacro

%macro u32 2
    %xdefine %$strucname %+ . %+ %1 %$strucoffset
    %xdefine %$strucname %+ _ %+ %1 %+ _bytes 4
    %assign %$strucoffset %$strucoffset + (4 * %2)
%endmacro

%macro u64 2
    %xdefine %$strucname %+ . %+ %1 %$strucoffset
    %xdefine %$strucname %+ _ %+ %1 %+ _bytes 8
    %assign %$strucoffset %$strucoffset + (8 * %2)
%endmacro

/**
 * [safe_set] / [safe_get]
 * Purpose: Read/Write to a field, asserting the register size matches the field type.
 * Usage: 
 *   safe_set rax, PageTableEntry, Present, dl
 */
%macro safe_set 4
    // %1 = base ptr (e.g. rax)
    // %2 = struct name (e.g. PageTableEntry)
    // %3 = field name (e.g. Present)
    // %4 = value reg (e.g. dl)
    ...
    %elif %2_%3_bytes == 8
        mov qword [%1 + %2.%3], %4
    %else
        %error "Safe Struct: Invalid or unaligned field write."
    %endif
%endmacro
...
%macro safe_get 4
    // %1 = dest reg (e.g. dl)
    // %2 = base ptr (e.g. rax)
    // %3 = struct name (e.g. PageTableEntry)
    // %4 = field name (e.g. Present)
    ...
    %elif %3_%4_bytes == 8
        mov %1, qword [%2 + %3.%4]
    %else
        %error "Safe Struct: Invalid or unaligned field read."
    %endif
%endmacro
